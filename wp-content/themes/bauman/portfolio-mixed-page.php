...
$bauman_max_items		= bauman_get_post_meta( BAUMAN_THEME_OPTIONS, get_the_ID(), 'bauman-opt-page-portfolio-mixed-items' );
if( empty($bauman_max_items) ){
	$bauman_max_items = 1000;
}
					...
					<div class="portfolio-wrap <?php if( $bauman_enable_spacing ) { echo "spaced "; } ?><?php echo esc_attr( $bauman_caption_type ); ?>">                
						...
						<?php
							...
							while( $bauman_portfolio->have_posts() ){
								...
								if( $bauman_items_count > $bauman_max_items ){
									
									// if the portfolio items is not a secondary item
									set_query_var('bauman_query_var_is_secondary_item', true);
								}
